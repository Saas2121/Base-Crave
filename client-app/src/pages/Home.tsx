        ...
        <header className={styles.header}>
          <div className={styles.welcomeRow}>
            ...
            <div className={styles.welcomeText}>
              <p className={styles.greeting}>Hey, {user?.name}!</p>
          ...
          {loading ? (
            ...
            <>
              <section className={styles.section}>
                ...
                <div className={styles.cardList}>
                  {(showAllTrending ? trendingPacks : trendingPacks.slice(0, 3)).map((pack) => (
                    <div key={pack.id} className={styles.card} onClick={() => navigate(`/product/${pack.id}`)}>
                      ...
                      <div className={styles.cardBody}>
                        <div className={styles.cardTitleRow}>
                          ...
                          <div className={styles.cardActions}>
                            {pack.pack_type === 'surprise' && (
                              <span className={`${styles.starBtn} ${favoritePackIds.includes(pack.id) ? styles.starBtnActive : ''}`}>
                                <svg className={styles.starIcon} viewBox="0 0 24 24" fill="none" strokeWidth="2">
                                  <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/>
                                </svg>
                              </span>
                            )}
                            <button className={`${styles.favBtn} ${favoritePackIds.includes(pack.id) ? styles.favBtnActive : ''}`} onClick={(e) => { e.stopPropagation(); toggleFavorite(pack.id) }}>
                        ...
                        <div className={styles.cardMeta}>
                          <span className={styles.cardMetaItem}>
                            <svg className={styles.metaIcon} viewBox="0 0 24 24" fill="none" stroke="#99a1af" strokeWidth="2">
                ...
                {trendingPacks.length > 3 && (
                  <button className={styles.expandBtn} onClick={() => setShowAllTrending((prev) => !prev)}>
                ...
                <div className={styles.spotsGrid}>
                  {topSpots.map((spot) => (
                    <div key={spot.id} className={styles.spotCard} onClick={() => navigate('/search/list')}>
                      <div className={styles.spotAvatar}>{spot.name.slice(0, 1).toUpperCase()}</div>
                    </div>
                  ))}
                </div>
              </section>
                    ...
                    {fixedNewToday.map((pack) => (
                      <div key={pack.id} className={styles.carouselCard} onClick={() => navigate(`/product/${pack.id}`)}>
                        <div className={styles.carouselCardContent}>
                          <h4>{pack.title}</h4>
                          <p>{pack.store?.name}</p>
                          <span className={styles.carouselPrice}>${pack.price}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                ) : (
                  <p className={styles.emptyMsg}>No fixed packs new today.</p>
                )}
              </section>
              ...
              <section className={styles.section}>
                ...
                {surpriseAvailableNow.length > 0 ? (
                  <div className={styles.cardList}>
                    {surpriseAvailableNow.map((pack) => (
                      <div key={pack.id} className={styles.card} onClick={() => navigate(`/product/${pack.id}`)}>
                        <div className={styles.cardImageArea}>
                          <span className={styles.cardImagePlaceholder}>S</span>
                          <button className={styles.favBtn} onClick={(e) => { e.stopPropagation(); toggleFavorite(pack.id) }}>
                        ...
                        <div className={styles.cardBody}>
                          <h3 className={styles.cardTitle}>{pack.store?.name}</h3>
                          <span className={styles.cardPrice}>${pack.price}</span>
                          <div className={styles.cardMeta}>
                            <span className={styles.cardMetaItem}>
                              <svg className={styles.metaIcon} viewBox="0 0 24 24" fill="none" stroke="#99a1af" strokeWidth="2">
                                <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/>
                  ...
                  <p className={styles.emptyMsg}>No surprise packs available now.</p>
                )}
              </section>
            </>
          )}
