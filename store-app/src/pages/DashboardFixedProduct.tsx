    ...
    <div className={styles.dashboard}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className={styles.container14}>
          <div className={styles.label2}>
            <div className={styles.packName}>Pack Type</div>
          </div>
          <div className={styles.container15}>
            <div className={styles.button} onClick={() => navigate('/dashboard/surprise')} style={{ cursor: 'pointer' }}>
              <div className={styles.container16}>
                <div className={styles.div3}>🎁</div>
              </div>
              <div className={styles.container17}>
                <div className={styles.packName}>Surprise</div>
              </div>
            ...
            <div className={styles.button2}>
              ...
              <div className={styles.container17}>
                <div className={styles.packName}>Fixed</div>
              </div>
              <div className={styles.container18}>
        ...
        <div className={styles.container22}>
          <div className={styles.container23}>
            <div className={styles.label4}>
              <div className={styles.packName}>Details</div>
            </div>
            <div className={styles.textInput2}>
              <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="e.g., It includes three bread rolls and a cake." rows={4} style={{ width: '100%', height: '100%', background: 'transparent', border: 'none', resize: 'none', fontSize: '16.98px', fontFamily: 'General Sans', color: '#ffffff', outline: 'none', gridColumn: 1, gridRow: 1, lineHeight: 1.4 }} />
            </div>
          </div>
        </div>
        ...
        <div className={styles.textInput3}>
          <input type="number" min={1} value={price} onChange={(e) => setPrice(e.target.value)} placeholder="$42.000" required style={{ width: '100%', background: 'transparent', border: 'none', padding: 0, fontSize: '16.98px', fontFamily: 'General Sans', color: '#ffffff', outline: 'none' }} />
        </div>
        ...
        {success && <p style={{ position: 'absolute', top: '1640px', left: '39px', color: '#15703d', background: '#ebfaee', border: '1px solid #bde8c7', borderRadius: '8px', padding: '8px 12px', fontSize: '13px', zIndex: 5 }}>{success}</p>}
        {error && <p style={{ position: 'absolute', top: '1640px', left: '39px', color: '#a23c31', background: '#fff1f0', border: '1px solid #f5c5bf', borderRadius: '8px', padding: '8px 12px', fontSize: '13px', zIndex: 5 }}>{error}</p>}
        <div className={styles.button5} style={{ cursor: creating ? 'not-allowed' : 'pointer', opacity: creating ? 0.6 : 1 }}>
          <button type="submit" className={styles.createPack} disabled={creating} style={{ position: 'absolute', top: '16.04px', left: 'calc(50% - 46.13px)', lineHeight: '24.05px', fontWeight: 600, background: 'none', border: 'none', color: '#fff', cursor: 'inherit', padding: 0, fontSize: '16.04px' }}>{creating ? 'Creating...' : 'Create Pack'}</button>
        </div>
      </form>
      {showTimePicker && (
        <TimePickerModal
